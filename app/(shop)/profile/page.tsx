    ...
    <div className="space-y-4 sm:space-y-6">
        ...
        <CardContent className="p-4 sm:p-6 md:p-8">
          <div className="flex flex-col md:flex-row gap-6 sm:gap-8">
            ...
            <div className="flex-1 space-y-4 sm:space-y-6">
              <div>
                <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100 mb-1">{user.name}</h2>
                ...
                {user.phone && (
                  <div className="flex items-start gap-3 p-3 sm:p-4 bg-gray-50 dark:bg-gray-700 rounded-xl border border-gray-100 dark:border-gray-600">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs sm:text-sm font-medium text-gray-900 dark:text-gray-100">{user.phone}</p>
                    </div>
                  </div>
                )}
              </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        <StatCard
          ...
          t={t}
        />
        <StatCard icon={<Heart />} label={t("wishlist")} value={user.wishlist?.length || 0} color="pink" t={t} />
        <StatCard icon={<MapPin />} label={t("myAddresses")} value={user.addresses?.length || 0} color="purple" t={t} />
        <StatCard icon={<Award />} label={t("rating")} value="0" color="amber" t={t} />
      </div>
...
function StatCard({
  ...
  return (
    <motion.div whileHover={{ y: -4, scale: 1.02 }} transition={{ duration: 0.2, ease: "easeOut" }}>
      <Card className="border-0 shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group relative dark:bg-gray-800 dark:border-gray-700">
        <CardContent className="p-4 sm:p-6 relative z-10">
          ...
          <div className="space-y-1 sm:space-y-2">
            <p className="text-xs sm:text-sm font-semibold text-gray-600 dark:text-gray-400 tracking-wide">{label}</p>
            {loading ? (
